...
const FAQ = () => {
  ...
  return <section id="faq" className="py-16 bg-carteBackground/50" ref={elementRef as React.RefObject<HTMLDivElement>}>
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-8 relative">
          {/* Decorative elements */}
          <div className="absolute top-0 -left-16 w-32 h-32 bg-carteYellow/10 rounded-full filter blur-xl animate-float" style={{
          animationDelay: '0s'
        }}></div>
          <div className="absolute bottom-20 -right-16 w-40 h-40 bg-cartePink/10 rounded-full filter blur-xl animate-float" style={{
          animationDelay: '1s'
        }}></div>
          <div className="absolute top-1/3 right-0 w-24 h-24 bg-carteYellow/20 rounded-full filter blur-lg animate-float" style={{
          animationDelay: '0.5s'
        }}></div>
          ...
          <div className={`bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg border border-carteYellow/20 overflow-hidden transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <Accordion type="single" collapsible className="w-full">
              {faqItems.map((item, i) => <AccordionItem value={`item-${i}`} key={i} className="border-b border-carteYellow/20 last:border-0">
        ...
        <div className={`mt-12 transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="bg-gradient-to-r from-cartePink/90 to-carteYellow/90 rounded-2xl p-6 md:p-8 shadow-lg flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
              <a href="#" className="kawaii-button bg-white text-cartePink hover:bg-gray-100 flex justify-center items-center gap-2 w-full md:w-auto">
