...
const FeatureCard = ({ icon, title, description, delay, isVisible }: FeatureCardProps) => {
  return (
    <div 
      className={`bg-brown-400/20 rounded-2xl p-8 hover-lift ${
        isVisible 
          ? `opacity-100 translate-y-0 transition-all duration-700 ease-out delay-${delay}` 
          : 'opacity-0 translate-y-10'
      }`}
    >
      <div className="w-16 h-16 bg-teal-600/10 rounded-xl flex items-center justify-center text-teal-700 mb-6">
        {icon}
      </div>
